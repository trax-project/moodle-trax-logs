...
class store_test extends test_config {
    ...
    public function test_all_events() {
        ...
        $triggers[] = \core\event\user_loggedin::create([
            'objectid' => $user->id,
            'other' => ['username' => $user->username],
        ])->trigger();

        $triggers[] = \core\event\course_viewed::create([
            'context' => context_course::instance($course->id),
        ])->trigger();

        $book = $this->getDataGenerator()->create_module('book', array('course' => $course->id));
        $triggers[] = \mod_book\event\course_module_viewed::create([
            'objectid' => $book->id,
            'context' => context_module::instance($book->cmid),
        ])->trigger();

        $chat = $this->getDataGenerator()->create_module('chat', array('course' => $course->id));
        $triggers[] = \mod_chat\event\course_module_viewed::create([
            'objectid' => $chat->id,
            'context' => context_module::instance($chat->cmid),
        ])->trigger();

        $choice = $this->getDataGenerator()->create_module('choice', array('course' => $course->id));
        $triggers[] = \mod_choice\event\course_module_viewed::create([
            'objectid' => $choice->id,
            'context' => context_module::instance($choice->cmid),
        ])->trigger();

        $data = $this->getDataGenerator()->create_module('data', array('course' => $course->id));
        $triggers[] = \mod_data\event\course_module_viewed::create([
            'objectid' => $data->id,
            'context' => context_module::instance($data->cmid),
        ])->trigger();

        $feedback = $this->getDataGenerator()->create_module('feedback', array('course' => $course->id));
        $triggers[] = \mod_feedback\event\course_module_viewed::create([
